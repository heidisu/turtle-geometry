# Welcome to Fun with F# and turtle geometry! 🎉

<p style="font-size:40px">https://github.com/heidisu/turtle-geometry</p>

## Before we start

* Clone the repository
* Follow the instructions in the getting started section

---

## Introduction

* Hi, I'm Heidi👋
* The motivation for the workshop
    * The book
    * Need more coding joy in life

---

## The goal 🎯

Have fun and maybe learn some F# and/or geometry

---

## The workshop

* F#
* Turtle
* Exploratory
    * Toggle arrow
    * Design choices and project setup
* F# interactive

---

## How to proceed

* The instructions are in [README.md](../README.md)
    * First task is to make a triangle
* GitHub issues
    * Share your work!
* Spend time on the parts you like, follow your whims
* Talk to your neighbours, work together if you like
* Ask questions and share ideas
* We might discuss some topics together. You can follow or continue your own path
* Use AI as a learning partner
