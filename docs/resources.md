# Resources

## F#

* [School of .NET](https://github.com/nrkno/dotnetskolen/blob/main/README_EN.md) - A workshop in F# that learns you how to make a proper API with test projects, API contracts, and domain driven design.
* [F# for Fun and Profit](https://fsharpforfunandprofit.com) - Scott Wlashin's amazing web page on everything F#, from beginner to advanced topics. He has also written the great book [Domain Modeling Made Functional](https://fsharpforfunandprofit.com/books/), and has given many [presentations](https://fsharpforfunandprofit.com/video/) on F#.
 
## Turtle geometry

* [Abelson and diSessa: Turtle Geometry: The Computer as a Medium for Exploring Mathematics](https://direct.mit.edu/books/oa-monograph/4663/Turtle-GeometryThe-Computer-as-a-Medium-for)
* [Wikipedia: Logo programming language](https://en.wikipedia.org/wiki/Logo_(programming_language))
* [Wikipedia: Lindenmayer system](https://en.wikipedia.org/wiki/L-system)

## Turtle geometry in F#

* Scott Wlashin has written a blog post series on [13 ways of looking at a turtle](https://fsharpforfunandprofit.com/posts/13-ways-of-looking-at-a-turtle/).

## Hilbert curve

* [Wikipedia: Hilbert curve](https://en.wikipedia.org/wiki/Hilbert_curve)
* [Hilbert: Über die stetige Abbildung einer Linie auf ein Flächenstück (PDF)](https://webhomes.maths.ed.ac.uk/~v1ranick/papers/hilbert.pdf)
* [Hayes: Crinkly curves](https://www.americanscientist.org/article/crinkly-curves)
* [Benjojo: Mapping the whole internet with Hilbert curves](https://blog.benjojo.co.uk/post/scan-ping-the-internet-hilbert-curve)
* [Kanungo: Space-Filling Curves](https://arxiv.org/html/2501.04705v1)
