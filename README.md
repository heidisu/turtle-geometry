...
The snowflake can also be generalised to any regular polygon of `n` sides. Try to make the functions `polyside` and `polyflake`, that take `n` as argument, in addition to `size` and `level`. `polyside` should recursively replace the middle segment with `n - 1` sides of the polygon. Since our turtle operates with integer angles, `n` must divide 360.
